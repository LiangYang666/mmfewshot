...
if __name__ == "__main__":
    src_labelme_dir = "/media/E_4TB/YL/mmlab/mmfewshot/mytools/data/可见光/飞机+裁剪"      # 原labelme路径
    dst_labelme_dir = "/media/E_4TB/YL/mmlab/mmfewshot/mytools/data/可见光/飞机+裁剪-scale"    # 新labelme路径
    ...
    img_files = [x for x in src_files if x.rsplit('.')[0]+'.json' in src_json_files and x.rsplit('.')[-1].lower() in img_types]
    for img_file in tqdm(img_files):
        img_path = os.path.join(src_labelme_dir, img_file)
        json_path = os.path.join(src_labelme_dir, img_file.rsplit('.')[0]+'.json')
        ...
        with open(os.path.join(dst_labelme_dir, img_file.rsplit('.')[0]+'.json'), 'w') as f:
